...
{
    // GW2's weekly raid reset lands on Monday 07:30 UTC. We post a single
    // reminder toast at that moment so users know to refresh their KP
    // progression / log a fresh week. Toast can be disabled in settings.
    //
    // Schedules a one-shot timer to the next reset boundary. After it
    // fires, the callback re-schedules itself for the following week.
    public sealed class WeeklyResetReminder : IDisposable
    {
        ...
        private void OnFire()
        {
            if (_settings.NotifyWeeklyReset.Value)
            {
                _stack.Push(new EventToast(
                    ...
                    subtitle: "Refresh your kill-proofs · new week is live",
        ...
        {
            // Monday 07:30 UTC. DayOfWeek.Monday == 1.
